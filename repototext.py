...
def process_csv_file(csv_file_path):
    ...
    for index, row in df.iterrows():
        repo_url = row['Source Code']
        print(f"Processing repository: {repo_url} ({index + 1}/{len(df)})")
        try:
            ...
            processed_text = f"{instructions}\nREADME:\n{readme_content}\n\n{repo_structure}\n\n{file_contents}"
            # Store the processed text in the new column
            df.at[index, 'proc text'] = processed_text
        except Exception as e:
            print(f"An error occurred while processing {repo_url}: {e}")

    # Save the updated DataFrame to a new CSV file
    output_csv_file_path = "processed_showcase_links_details.csv"
...
if __name__ == '__main__':
    if not GITHUB_TOKEN:
        ...
        print("Please create a .env file in the same directory with your GitHub token.")
        print("Example: GITHUB_TOKEN=your_token_here")
    else:
        csv_file_path = "showcase_links_details.csv"
